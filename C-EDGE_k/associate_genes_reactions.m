...
%external), thus respecting the common rule that AND is solved before OR
    ...
    if (isempty(findstr('max(', stringa)) && isempty(findstr('min(', stringa)) && ~strcmp(stringa,''))
        stringa_output = ['/' stringa '/'];  %we need other "/" because in this case the gene set is single gene, it also needs the / at beginning and at the end
    else
    ...
    function stringa = substitute(stringa,i)
        ...
        j = i;
        while (  (strcmp(stringa(j),'(')==0) || (parentesi_trovate~=-1) ) && strcmp(stringa(j),',')==0  && (j>1)   %si ferma se trova una parentesi ( che non e' intermedia. E' necessario questo altrimenti si fermerebbe alla prima parentesi ( trovata, anche se questa non e' quella relativa all'AND, ma appartiene ad esempio a un "sotto-OR" che costituisce il ramo sinistro dell'AND, cioe' nel caso ((C or D) and (B or E))
            j = j-1;
            if (stringa(j)==')')
        ...
        while ( (strcmp(stringa(k),')')==0) || ( parentesi_trovate~=-1 )) && strcmp(stringa(k),',')==0 && (k<length(stringa))    %si ferma se trova una parentesi ) che non e' intermedia. E' necessario questo altrimenti si fermerebbe alla prima parentesi ) trovata, anche se questa non e' quella relativa all'AND, ma appartiene ad esempio a un "sotto-OR" che costituisce il ramo sinistro dell'AND, cioe' nel caso ((C or D) and (B or E))
            k = k+1;
            if (stringa(k)=='(')
                parentesi_trovate = parentesi_trovate+1; %segnala altre parentesi trovate lungo il percorso
            end

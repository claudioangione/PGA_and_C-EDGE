...
init_out = evaluate_objective(x,M,V,fbarecon,geni,reaction_expression,pos_genes_in_react_expr,ixs_geni_sorted_by_length)   % we let FBA maximise oncoflux so we are sure that the oncoflux we get is the maximum possible with that particular gene expression profile
...
%% single gene epsilon perturbation (we can find the same result on the diagonal of the pairwise correlation matrix)
%%
...
parfor i = 1:numel(geni)
    ...
    x_eps = x; x_eps(i) = eps;
    out_eps = evaluate_objective(x_eps,M,V,fbarecon,geni,reaction_expression,pos_genes_in_react_expr,ixs_geni_sorted_by_length)   % we let FBA maximise oncoflux so we are sure that the oncoflux we get is the maximum possible with that particular gene expression profile
    out_eps(1) = -out_eps(1);
    out_eps(2) = -out_eps(2);

    x_ko = x; x_ko(i)=0;
    out_ko = evaluate_objective(x_ko,M,V,fbarecon,geni,reaction_expression,pos_genes_in_react_expr,ixs_geni_sorted_by_length)   % we let FBA maximise oncoflux so we are sure that the oncoflux we get is the maximum possible with that particular gene expression profile
    out_ko(1) = -out_ko(1);
    out_ko(2) = -out_ko(2);
...
%     disp([edge_bio(i) edge_onco(i) edge_bio_onco_hyperv(i)]); 
 ...
 save c-edge_scores.mat edge_bio 
...
%         value(end+1) = abs(edge_bio_pairwise(rows_pairwise_toxic(i),cols_pairwise_toxic(i)) - max(edge_bio(rows_pairwise_toxic(i)),edge_bio(cols_pairwise_toxic(i))));
...
%         value2(end+1) = abs(edge_bio_pairwise(rows_pairwise_toxic2(i),cols_pairwise_toxic2(i)) - max(edge_bio(rows_pairwise_toxic2(i)),edge_bio(cols_pairwise_toxic2(i))));
...
%     count = count+1;
%     x_eps([i j]) = eps;
%     out_eps_var(count,:) = evaluate_objective_EDGE(x_eps,M,V,fbarecon,geni,reaction_expression,pos_genes_in_react_expr,ixs_geni_sorted_by_length);   % we let FBA maximise oncoflux so we are sure that the oncoflux we get is the maximum possible with that particular gene expression profile
% end

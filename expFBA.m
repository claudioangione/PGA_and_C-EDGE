...
pos_genes_in_react_expr = load('pos_genes_in_react_expr.mat');
pos_genes_in_react_expr = pos_genes_in_react_expr.pos_genes_in_react_expr;
ixs_geni_sorted_by_length = load('ixs_geni_sorted_by_length.mat');
ixs_geni_sorted_by_length = ixs_geni_sorted_by_length.ixs_geni_sorted_by_length;
...
%    chromosome = chromosome + 0.1.*(rand(pop,V+M)-0.5.*ones(pop,V+M));  %adds some initial random noise
     chromosome = chromosome + rand(pop,V+M) - rand(pop,V+M);   %adds some initial random noise, random in every cell
...
%     chromosome(i,V+3)= sum(abs(chromosome(i,:)-ones(1,numel(chromosome(i,:))))); % 3rd obj: minimize the gene overexpression and underexpression (which is the distance from 1)
    ...
    parfor i=1:pop
    %for i=1:pop
        aux(i,:) = evaluate_objective(chromosome(i,1:V), M, V, fbarecon, geni, reaction_expression, pos_genes_in_react_expr, ixs_geni_sorted_by_length);
    end
...
%     else
        ...
        M, V, mu, mum, min_range, max_range, fbarecon, geni, reaction_expression, pos_genes_in_react_expr, ixs_geni_sorted_by_length);
%     end

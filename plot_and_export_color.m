...
% for the minimisation of the objectives, so when we have maximized something, its values were all negatives!
others(:,1)= + others(:,1);
others(:,2)= - others(:,2);
...
%% PLOT NON-DOMINATED ON THE SAME PLOT
...
%K = convhull([x y z]);  %convex hull of all the Pareto-optimal points, so we can plot the Pareto optimal surface trisurf
%trs = trisurf(K,x,y,z,C);
...
aux = xlim;
%xlim([min(x) max(x)+0.05*max(x)]);
xlab = get(gca,'xtick');

% vers = version; %matlab version
% if ~isempty(strfind(vers, 'R2014b'))
%     set(gca,'xticklabel',cellstr(sprintf('%.2f\n',xlab))); %for Matlab r2014b
% else
%     set(gca,'XTickLabel',sprintf('%.2f|',xlab)); %for Matlab r2014a and previous versions
% end
...
title('');
xlabel('Biomass [h^{-1}]');
ylabel('Phosphoglycerate dehydrogenase [mmol h^{-1} gDW^{-1}]');
...
%% PLOT NON-DOMINATED ONLY IN A NEW FIGURE
fig_nondom = figure;
...
%z = non_dominated(:,3);
C = non_dominated(:,M+2);
%scatter3(x,y,z,40,[0.5 0 0],'*');
scatter(x,y,40,'blue','o');
%grid on
hold on
...
[h_m h_i]=inset(fig_nondom,fig_others);
colormap(jet(256)) 
colorbar;
colormap(flipud(colormap)) %reverses the colormap
box on
